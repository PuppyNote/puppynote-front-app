...
export default function TopBar({ navigation, options, route, title: manualTitle }: any) {
  const [hasNotification, setHasNotification] = useState(false);
  const insets = useSafeAreaInsets();
  const title = manualTitle || options?.headerTitle || route?.name || 'PuppyNote';
  ...
  // 메인 탭 화면인지 확인
  const mainTabs = ['Home', 'Walk', 'Supplies', 'Community', 'Food', 'Settings'];
  const isMainTab = route?.name ? mainTabs.includes(route.name) : false;
